...
export ISTIO_OUT_LINUX:=$(OUT_DIR)/linux_$(GOARCH_LINUX)/$(BUILDTYPE_DIR)
...
export ISTIO_ENVOY_LINUX_DEBUG_DIR ?= ${OUT_DIR}/linux_${GOARCH_LINUX}/debug
...
export ISTIO_ENVOY_LINUX_RELEASE_DIR ?= ${OUT_DIR}/linux_${GOARCH_LINUX}/release
...
.PHONY: build-linux
build-linux: depend
	STATIC=0 GOOS=linux GOARCH=$(GOARCH_LINUX) LDFLAGS='-extldflags -static -s -w' common/scripts/gobuild.sh $(ISTIO_OUT_LINUX)/ $(BINARIES)
...
$(ISTIO_OUT_LINUX)/$(shell basename $(1)):
	STATIC=0 GOOS=linux GOARCH=$(GOARCH_LINUX) LDFLAGS='-extldflags -static -s -w' common/scripts/gobuild.sh $(ISTIO_OUT_LINUX)/ $(1)
endif
endef
